...
insiders_dir = Path("csv_testing/insiders_fourth")
securities_dir = Path("csv_testing/securities_fourth")
tracking_csv = Path("gemini_results_fourth.csv")
...
def extract_from_md(md: str, name: str) -> tuple[str, str, str]:
 ...
 You are analyzing a U.S. Federal Reserve FR Y-6 regulatory filing.
    ...
    - Do not put things from the securites or shareholders table (item 3) inside the insiders table. Insiders information should only come from insiders table
...
"""
    ...
    s3.upload_file(str(insiders_path), bucket_name, f"csv_testing/insiders_fourth/{name}.csv")
    s3.upload_file(str(shareholders_path), bucket_name, f"csv_testing/securities_fourth/{name}.csv")
